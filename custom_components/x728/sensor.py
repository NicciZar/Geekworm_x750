...
async def async_setup_platform(hass, config, async_add_entities,
                               discovery_info=None):
    """Set up the X728 sensor."""
    name = config.get(CONF_NAME)

    sensor_handler = await hass.async_add_job(_setup_X728, config)
    if sensor_handler is None:
        return

    dev = []
    for variable in config[CONF_MONITORED_CONDITIONS]:
        dev.append(X728Sensor(
            sensor_handler, variable, SENSOR_TYPES[variable][1], name))

    async_add_entities(dev)
    return

def _setup_X728(config):
    """Set up and configure the X728 sensor."""
    ...
    try:
        ...
        sensor = X728(i2c_address, bus)

    except (RuntimeError, IOError):
        _LOGGER.error("X728 sensor not detected at 0x%02x", i2c_address)
        return None

    sensor_handler = X728Handler(
        sensor
    )

    sleep(0.5)  # Wait for device to stabilize
    if not sensor_handler.sensor_data.voltage:
        _LOGGER.error("X728 sensor failed to Initialize")
        return None

    return sensor_handler

class X728Handler:
    """X728 sensor working in i2C bus."""
    ...
    def __init__(
        ...
        self.sensor_data = X728Handler.SensorData()
        self._sensor = sensor
...
class X728Sensor(Entity):
    """Implementation of the X728 sensor."""

    def __init__(self, X728_client, sensor_type, temp_unit, name):
        """Initialize the sensor."""
        self.client_name = name
        self._name = SENSOR_TYPES[sensor_type][0]
        self.X728_client = X728_client
    ...
    async def async_update(self):
        """Get the latest data from the X728 and update the states."""
        await self.hass.async_add_job(self.X728_client.update)
        if self.type == SENSOR_VOLTAGE:
            self._state = round(self.X728_client.sensor_data.voltage, 2)
        elif self.type == SENSOR_CAPACITY:
            self._state = round(self.X728_client.sensor_data.capacity, 2)

class FieldData:
    """Structure for storing X728 sensor data."""
...
class X728Data:
    """Structure to represent X728 device."""
